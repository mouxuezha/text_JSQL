...
class StagePrompt:
    ...
    def get_stage_prompt_red(self, time_step):
        ...
        if self.stage_now == "机动":
            prompt = prompt+ "请命令各单位搜索前进，向经纬度(100.096, 13.6109)处集结，步兵不下车，无人机探测夺控点附近"
        elif self.stage_now == "交战":
            prompt = prompt+  "请命令各单位沿地图西侧推进，到(100.092, 13.6463)，自行迫榴炮占据有利位置(100.114, 13.631)。"
        elif self.stage_now == "推进":
            prompt = prompt+  "请命令除自行迫榴炮以外的地面装备向北突击，向经纬度(100.124, 13.661)附近展开搜索攻击"
