...
class TTS_interface():
    ...
    def __init__(self,**kargs):
        ...
        # 要用多个的话等后面再来改罢。        
        pass
    def check_config(self,key_str):
        # 检测一下config里面这个是多少。
        value_config = self.config[key_str]
        return value_config
    ...
    def pop_status_list(self):
        ...
        if geshu>0:
            # 那就是没毛病，可以正常取
            status_jieguo = copy.deepcopy(self.status_list[0])
            if geshu>self.min_list_num:
                # 如果不是太少，就删去一些时效性差的。
                del self.status_list[0]
                # 这有逻辑漏洞，如果个数只有一个或者很少，它就不会删除，就会一直循环生成。得去生成的线程里面处理一下。
        else:
            status_jieguo = ""

        return status_jieguo
    
    def get_debug_pcm_name_list(self):
        # 这个是用来分开测试两个线程的。
        for i in range(5):
            self.index_generate = self.index_generate + 1
            name_new = "jieshuo"+str(self.index_generate) + ".pcm"
    ...
    def run_mul(self):
        ...
        #         
        # debug :
        # self.pcm_name_list = self.get_debug_pcm_name_list()
        
        if self.check_config("flag_generate"):
            self.thread1 = threading.Thread(target=self.run_generate)
        if self.check_config("flag_play"):
            if not(self.check_config("flag_generate")):
                self.pcm_name_list = self.get_debug_pcm_name_list()
            self.thread2 = threading.Thread(target=self.run_play)
    ...
    def run_generate(self):
        ...
        while True:
            ...
            if len(self.status_list)<self.min_list_num:
                # 这个是用来防止一直循环生成最后几句还删不掉的。
                time.sleep(1)
                print("run_generate: waiting for more text")  
                continue 
            ...
            if status_str == "":
                # 如果还没有取到的态势，那就直接返回，无事发生。
                time.sleep(1)
                continue
            else:
                self.index_generate = self.index_generate + 1
                time.sleep(1) # 这个是调试的时候用来控制一下速度的。
            ...
            tongxin_thread = self.TTS_generator.text_to_voice(response_str)
            # 这里有说法了，text_to_voice里面本来就做了多线程，所以这个函数是不阻塞的，但是这里希望它阻塞，不然就会一直生成第一句。
            tongxin_thread.join() # 所以要把那个线程返回出来然后等待结束。
            self.pcm_name_list.append(name_new)
    ...
    def run_play(self):
        ...
        while True:
            ...
            if geshu_generated>0:
                ...
                # 那就是列表里没得可说，那就先放一下。
                time.sleep(1)
                print("run_play: waiting for more pcm file")  
                continue
            pass 
...
def main_loop_demo():
    config = {"flag_generate":True,"flag_play":True}
    shishi_TTS = TTS_interface(config =config )
    shishi_TTS.run_mul()
    text_demo_list = get_text_demo_list()
    for i in range(5):
        text_demo_single = text_demo_list[i]
        shishi_TTS.add_status_list(text_demo_single)
        time.sleep(1.5)
    pass 
