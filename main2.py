...
class auto_run_comunicator():
    ...
    def __init__(self):
        # 初始化一系列的通信啥的东西。
        self.config = {"red_ip":"192.168.1.114", "red_port": "20001",
                    "blue_ip": "192.168.1.114", "blue_port": "20002","flag_server_waiting":True,"dialog_box_model": "QtC++","socket_debug_model":"net_debug"}
        self.__init_server()
    ...
    def __init_server(self):
        ...
        config = self.config
        widget = MyWidget_server(config=config)
    ...
    def __init_client_red(self):
        ...
        widget = MyWidget_client(role="red_player",config = config)
        widget.run_mul()        
        pass 
    ...
    def __init_client_blue(self):
        config = self.config    # dialog_box_model能选QtC++和QtPython，socket_debug_model有local_debug和net_debug，区别是用不用那个假的socket类。
        # 这个config其实是服务于command_processor的，不是服务于dialogbox，所以原则上其他的dialogbox也可以用。
        widget = MyWidget_client(role="blue_player",config = config)
        widget.run_mul()        
        pass 
